...
# Phase 4 (Option C): after a match is found, warm-transfer the caller to the
# doctor's office to confirm they're taking new patients. SMS (Option B) was
# tried and confirmed blocked (gated behind the same compliance approval as
# outbound dialing), so this replaces it.
OFFICE_TRANSFER_NUMBER = "+19303335065"
...
def on_find_provider_complete(call: guava.Call):
    ...
    if matches:
        ...
        # Phase 4 (Option C): warm-transfer to the doctor's office to confirm
        # they're taking new patients. This hands the call off, so there's no
        # wrap_up task afterward — the office picks up from here.
        logger.info("Transferring call %s to office %s", call.id, OFFICE_TRANSFER_NUMBER)
        call.transfer(
            destination=OFFICE_TRANSFER_NUMBER,
            instructions=(
                "Let the caller know you're transferring them to the doctor's "
                "office to confirm they're taking new patients."
            ),
        )
        return
